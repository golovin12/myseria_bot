    ...
    try:
        ...
        if data['publish_date'].isdigit():
            if int(data['publish_date']) < 366 and int(data['publish_date']) >= 0:
                ...
                await message.reply(
                    "Вы ввели некорректное число (Число должно быть в промежутке 0-365), попробуйте снова(или введите: 'отмена'): ")

        else:
            try:
                if datetime.strptime(data['publish_date'],
                                     '%d-%m-%Y').date() <= datetime.today().date() and datetime.strptime(
                    data['publish_date'], '%d-%m-%Y').date() >= (datetime.today().date() - timedelta(days=365)):
                    await message.reply("Подождите, собирается информация...")
                    serials = user_news(message.from_user.id, f"{data['publish_date']}")
                    if len(serials) == 0:
                        await bot.send_message(message.from_user.id, "Новинок нет")
...
async def add_serials_command(message: types.Message, state: FSMContext):
    try:
        ...
        await message.reply(
            "Введи названия сериалов через ':> ', для добавления в список отслеживания(Пример: Игра в кальмара:> Игра престолов):\n"
    ...
    try:
        ...
        if data['command'] == "deleteserials":
            ...
            if otvet == "Ok":
                ...
                answer = "\n".join(otvet)
                await message.answer(
                    f"Вы указали не корректное имя сериала (Имя сериала должно точно совпадать с именем на сайте MySeria)\n"
